...
export const getSnapshot = async (
  ...
  console.warn(reserveAccountAddress);
  ...
  while (true) {
    ...
    const multipleAccountInfos = await connection.getMultipleAccountsInfo(
      ...
      { encoding: 'jsonParsed' } as any as Commitment
    );
    ...
    let amount = new BN(0);
    try {
      amount = stSolMintAccountInfo.data.parsed.info.supply;
    } catch (e) {
      const dec = spltoken.MintLayout.decode(stSolMintAccountInfo.data);
      amount = dec.supply;
    }
